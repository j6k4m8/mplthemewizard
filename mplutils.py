...
def parse_from_rc_text(rc_text: str):
    """Parse the rc text into a dictionary."""
    ...
    fail_on_error = False
    try:
        for line_no, line in enumerate(rc_text.split("\n"), 1):
            line = transform(line)
            ...
            if len(tup) != 2:
                # "Missing colon, line %d (%r)", line_no, line.rstrip("\n")
                errors.append(f"Missing colon, line {line_no} ({linerstrip})")
                _log.warning("Missing colon, line %d (%r)", line_no, line.rstrip("\n"))
                continue
            key, val = tup
            key = key.strip()
            val = val.strip()
            if key in rc_temp:
                # "Duplicate key, line %d (%r)", line_no, line.rstrip("\n")
                errors.append(f"Duplicate key, line {line_no} ({linerstrip})")
                _log.warning("Duplicate key, line %d (%r)", line_no, line.rstrip("\n"))
        ...
        if key in rcsetup._validators:
            if fail_on_error:
                ...
                try:
                    ...
                    print(msg)
                    errors.append(f"Bad value, line {line_no}: {msg}")
                    _log.warning(
                        "Bad value, line %d (%r): %s", line_no, line.rstrip("\n"), msg
                    )
...
or from the matplotlib source distribution""",
            ...
            errors.append(f"Bad key, line {line_no}: {key}")
